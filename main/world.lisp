...
(defun get-explained-and-unexplained-beliefs (wrld)
  (let* ((belief-test #'(lambda (b1 b2) (equal (belief-content (second b1))
                                               (belief-content (second b2)))))
         (wrld-blfs (remove-duplicates (get-beliefs wrld) :test belief-test))
         (explained-beliefs (mappend #'(lambda (j) (mapcar #'(lambda (b) (belief-id (second b)))
                                                           (get-supported-beliefs j (agent-beliefs wrld))))
                                     (get-justifications wrld)))
         (unexplained-beliefs (filter #'(lambda (b) (not (member (belief-id (second b)) explained-beliefs)))
                                      wrld-blfs)))
    (values explained-beliefs unexplained-beliefs)))
