        ...
        (dolist (refire '(t nil))
          (loop for i from 1 to repetitions
             do (clearmem)
               ...
               (setq *refire-justifications* refire)
               (setq *vocal* nil)
               (destructuring-bind (prec recall gt-pm-overall unexp-avg-overall)
                 ...
                 (format t "~A,~A,~A,~A,~A,~A,~A,~A~%"
                         cases (subseq pb 12) steps (if refire "Yes" "No")
                         prec recall gt-pm-overall unexp-avg-overall))))))))

(defun test-tracking-anomalies (repetitions)
  (format t "Cases,PickBelief,Steps,Refire,Precision,Recall,PartialAccuracy,Unexplained~%")
  (dolist (cases '("cases-gray50"))
    (dolist (pb '("pick-belief-maximally-grounded"))
      (dolist (steps '(5000))
        (dolist (refire '(t nil))
          (loop for i from 1 to repetitions
             do (clearmem)
